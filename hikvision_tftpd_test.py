...
class TftpdTest(unittest.TestCase):
    ...
    _TEST_RRQ = (b'\x00\x01' + b'digicap.dav' + b'\x00'        # request file digicap.dav
                 + b'octet' + b'\x00'                          # mode octet
                 + b'timeout' + b'\x00' + b'5' + b'\x00'      # RFC 2349 timeout = 5 seconds
                 + b'blksize' + b'\x00' + str(_BLOCK_SIZE).encode() + b'\x00')  # RFC 2348 block size

    _TEST_RRQ_DEFAULT_BLKSIZE = (b'\x00\x01' + b'digicap.dav' + b'\x00'
                                 + b'octet' + b'\x00')

    _LARGE_BUFFER_SIZE = 65536

    _BLKSIZE_OPTION = b'blksize\x00' + str(_BLOCK_SIZE).encode() + b'\x00'
    ...
    def test_two_block(self):
        ...
        repetitions = 1 + blocksize // len(string.ascii_letters)
        data = (string.ascii_letters * repetitions).encode()
        self._setup(data)
